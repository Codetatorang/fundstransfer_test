package ibf2022.batch2.paf;

import java.io.StringReader;
import java.util.Date;

import org.bson.Document;
import org.springframework.jdbc.support.rowset.SqlRowSet;

import ibf2022.batch2.paf.serverstub.models.Account;
import ibf2022.batch2.paf.serverstub.models.TransferInfo;
import jakarta.json.Json;
import jakarta.json.JsonObject;
import jakarta.json.JsonReader;

public class Utils {

	public static Account toAccount(SqlRowSet rs) {
		Account account = new Account();
		account.setFullName(rs.getString("full_name"));
		account.setBalance(rs.getFloat("balance"));
		return account;
	}

	public static TransferInfo toTransfer(String jsonStr) {
		TransferInfo transfer = new TransferInfo();
		JsonReader reader = Json.createReader(new StringReader(jsonStr));
		JsonObject json = reader.readObject();
		transfer.setSrcAcct(json.getString("srcAcct"));
		transfer.setDestAcct(json.getString("destAcct"));
		transfer.setAmount((float)json.getJsonNumber("amount").doubleValue());
		return transfer;
	}

	public static Document toDocument(TransferInfo transfer, String transactionId) {
		Document doc = new Document();
		doc.put("transactionId", transactionId);
		doc.put("date", new Date());
		doc.put("srcAcct", transfer.getSrcAcct());
		doc.put("destAcct", transfer.getDestAcct());
		doc.put("amount", transfer.getAmount());
		return doc;
	}
}